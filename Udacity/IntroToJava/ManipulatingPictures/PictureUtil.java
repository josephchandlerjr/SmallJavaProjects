...
public class PictureUtil
{
    ...
    public static Picture grayAndFlipLeftToRight( Picture pic)
    {
        ...
        int[][] grayLevels = pic.getGrayLevels(); // grayLevels[row][col]
        // TODO flip it left to right
	for (int row=0; row < grayLevels.length; row++)
	{
		for(int col=0; col < grayLevels[0].length/2; col++)
		{
			int colToSwap = grayLevels[0].length-1-col;
			int temp = grayLevels[row][col];
			grayLevels[row][col] = grayLevels[row][colToSwap];
			grayLevels[row][colToSwap] = temp;
		}
	}
	...
	int[][] flipped = new int[grayLevels[0].length][grayLevels.length];
        for (int row=0; row < grayLevels.length; row++)
        {
		for (int col=0; col < grayLevels[0].length; col++)
		{
			flipped[col][flipped[0].length-1-row] = grayLevels[row][col];
		}

	}	
        return new Picture(flipped); //just so draft will compile. You will change it in the final
    }
